        ...
        let (state, input, output) = stmtConfig in
         let evalInstr = function 
          | BINOP op   -> 
            ...
            ((Value.of_int value ) :: tail, stmtConfig)
            | _              -> failwith "BINOP"
            )
          ...
          | STA (n, s)      ->
              ...
              (tail, (Stmt.update state n value idxs, input, output))
          | SEXP (tag, count) -> let values, rest = split count stack in ((Value.sexp tag @@ values)::rest, stmtConfig) 
          ...
          | BEGIN (_, params, locals) ->
              ...
              let state'' = List.fold_left (fun s (x, v) -> State.update x v s) state' (List.combine params @@ values) in
...
let rec match_pattern lFalse depth = function
  ...
  | Stmt.Pattern.Sexp (t, ps) -> true, [TAG t] @ [DROPJMP (lFalse, depth)] @
...
let rec compileLabel p lastLabel =
  ...
  | Expr.Array a          ->
    let compiled = List.flatten (List.rev (List.map expr a)) in
  ...
  | Expr.Call (f, args) ->
    let compileArgs = List.flatten (List.rev (List.map expr args)) in
    compileArgs @ [CALL (f, List.length args, false)]
  | Expr.Sexp (t, xs)     ->
    let compiled = List.flatten (List.rev (List.map expr xs)) in
  ...
  | Stmt.Assign (name, idxs, e) -> (
      match idxs with
        ...
        | idxs -> 
          let compiled_idxs = List.fold_left (fun comp e -> comp @ (expr e)) [] idxs in
          (List.rev compiled_idxs) @ expr e @ [STA (name, List.length idxs)], false) 
  | Stmt.Skip          -> [], false
  | Stmt.If (e, s1, s2)  ->
