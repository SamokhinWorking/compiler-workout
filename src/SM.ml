...
let rec eval env config programm  =
  ...
  | instr :: rest -> 
    ...
    match instr with
      ...
      | BEGIN (_,n, a) -> let stateWithScope = State.enter state (n @ a) in
      ...
      | CALL (f,_,_) -> evalJmp ((rest,state) :: controlStack, stack,stmtConfig) f
...
let rec compileLabel p lastLabel =
  ...
  | Expr.Call (f,args) ->
    (List.concat (List.map expr (List.rev args))) @ [CALL (f,List.length args, true)]
  in match p with
  | Stmt.Seq (s1, s2)  ->
  ...
  | Stmt.Call (f,args) ->
    (List.concat (List.map expr (List.rev args))) @ [CALL (f,List.length args, true)],false
